...
int main() {
    ...
    SinglyLinkedList<int> list1 = { 10, 20, 30, 40, 50 };
    ...
    SinglyLinkedList<int> list3(std::move(list2));
    ...
    // 13. Демонстрация прямого обхода (особенность односвязного списка)
    std::cout << "13. Демонстрация прямого обхода:\n";
    SinglyLinkedList<int> demoList = { 1, 2, 3, 4, 5 };
    std::cout << "Список: " << demoList << "\n";
    std::cout << "Прямой обход: [";
    for (size_t i = 0; i < demoList.getCount(); ++i) {
        std::cout << demoList[i];
        if (i < demoList.getCount() - 1) std::cout << " -> ";
    }
    std::cout << "]\n\n";
